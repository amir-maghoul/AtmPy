...
from atmpy.physics.thermodynamics import Thermodynamics # For Msq calculation, though not strictly needed for this simple case
...
class SineWaveAdvection1D(BaseTestCase):
    ...
    def __init__(self):
        ...
        self.advection_speed: float = 1.0  # u0
        ...
        self.background_rhoY: float = 1.0 # Background for the 'tracer' rhoY
    ...
    def setup(self):
        ...
        temporal_updates = {
            ...
            "dtfixed": 0.001, # Will be overridden by CFL if use_cfl_dt is true
            "use_cfl_dt": True,
        ...
        # --- Physics Settings ---
        physics_updates = {
            "wind_speed": [self.advection_speed, 0.0, 0.0], # Background flow is the advection speed
        ...
        print(f"1D Sine Wave Advection configuration complete. Msq = {self.config.model_regimes.Msq}")
        print(f"Output files: {self.config.outputs.output_base_name}{self.config.outputs.output_suffix}")
    ...
    def initialize_solution(self, variables: "Variables", mpv: "MPV"):
        ...
        variables.cell_vars[inner_slice_x + (VI.RHOU,)] = rho_initial * self.advection_speed

        # VI.RHOV, VI.RHOW (should be zero for 1D)
        if grid.ndim > 1 and VI.RHOV < variables.num_vars_cell: # Check if RHOV exists
            variables.cell_vars[inner_slice_x + (slice(None),) * (grid.ndim -1) + (VI.RHOV,)] = 0.0
        if grid.ndim > 2 and VI.RHOW < variables.num_vars_cell: # Check if RHOW exists
             variables.cell_vars[inner_slice_x + (slice(None),) * (grid.ndim -1) + (VI.RHOW,)] = 0.0
        ...
        rhoY_initial = self.background_rhoY + sine_perturbation # Could be different: 0.8*sine_perturbation
        ...
        mpv.state(gravity, Msq) # This calculates S0, S, hydrostate.cell_vars and hydrostate.node_vars
...
if __name__ == "__main__":
    ...
    grid = config.grid
    mpv = MPV(grid)
    num_vars_cell = max(VI.RHO, VI.RHOU, VI.RHOV, VI.RHOW, VI.RHOX, VI.RHOY) + 1 # Ensure enough vars
    ...
    nodal_bc_contexts_1d = (
                [BCApplicationContext(is_nodal=True)] * grid.ndim * 2
        )
    ...
    manager.apply_boundary_on_single_var_direction(mpv.p2_nodes, "x", nodal_bc_contexts_1d)
    ...
    # Plot RHO
    axs[0].plot(x_coords_full, variables.cell_vars[:, VI.RHO], 'bo-', label='Rho (full domain)', markersize=3)
    axs[0].plot(x_coords_inner, variables.cell_vars[tuple(inner_slice_x) +(VI.RHO,)], 'r.--', label='Rho (inner domain)')
    ...
    # Plot RHOY
    axs[1].plot(x_coords_full, variables.cell_vars[:, VI.RHOY], 'go-', label='RhoY (full domain)', markersize=3)
    axs[1].plot(x_coords_inner, variables.cell_vars[tuple(inner_slice_x) +(VI.RHOY,)], 'm.--', label='RhoY (inner domain)')
